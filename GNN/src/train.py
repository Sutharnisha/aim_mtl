...
with an MPNN backbone trained from scratch on QM9, on the paper's full
11-task QM9 property set (see data.py), at the paper's 10k-subset setting.
...
def train(
    ...
    seed:             int   = 42,
    head_hidden:      int   = 64,
    trainable_layers: int   = -1,    # -1 = all layers trainable (GNN has no pretrained weights)
    lr_model:         float = 5e-4,  # paper grid: {1e-3, 5e-4}
...
if __name__ == "__main__":
    ...
    p.add_argument("--trainable_layers", type=int,   default=-1,
                   help="-1 = all layers trainable (default; GNN has no pretrained weights)")
    p.add_argument("--lr_model",         type=float, default=5e-4,
                   help="Paper grid: {1e-3, 5e-4}")
